...
Bool hwc_init_hybris_native_buffer(ScrnInfoPtr pScrn)
{
    ...
    if (strstr(eglQueryString(renderer->display, EGL_EXTENSIONS), "EGL_HYBRIS_native_buffer2") == NULL)
    {
        xf86DrvMsg(pScrn->scrnIndex, X_ERROR, "EGL_HYBRIS_native_buffer2 is missing. Make sure libhybris EGL implementation is used\n");
        return FALSE;
    }
